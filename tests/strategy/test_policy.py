...
def test_up_regime_throttles_long_side() -> None:
    """Test UP regime throttles LONG (counter-trend) side."""
    ...
    # Apply policy for UP regime
    shaped = PlacementPolicy.shape_ladders(original, Regime.UP, cfg)

    # Should have only SHORT ladder (UP regime returns single ladder from GridEngine)
    # But when starting with SIDEWAYS ladders, we throttle the LONG side
    ...
    # LONG should be throttled to counter_levels
    assert len(long_ladder) == 2

    # SHORT should remain full
    assert len(short_ladder) == 5


def test_up_regime_keeps_short_full() -> None:
    """Test UP regime keeps SHORT (with-trend) ladder full."""
    cfg = create_test_config(counter_levels=3, grid_levels_short=7)
    mid = 100.0
    ...
    # SHORT should be unchanged (full 7 levels)
    assert len(short_ladder) == 7


def test_up_regime_scales_long_quantities() -> None:
    """Test UP regime scales LONG quantities by counter_qty_scale."""
    ...
    for i in range(len(long_shaped)):
        expected_qty = long_original[i].qty * 0.5
...
def test_down_regime_throttles_short_side() -> None:
    """Test DOWN regime throttles SHORT (counter-trend) side."""
    cfg = create_test_config(counter_levels=2)
    mid = 100.0
    ...
    # SHORT should be throttled to counter_levels
    assert len(short_ladder) == 2

    # LONG should remain full
    assert len(long_ladder) == 5


def test_down_regime_keeps_long_full() -> None:
    """Test DOWN regime keeps LONG (with-trend) ladder full."""
    cfg = create_test_config(counter_levels=3, grid_levels_long=8)
    mid = 100.0
    ...
    # LONG should be unchanged (full 8 levels)
    assert len(long_ladder) == 8


def test_down_regime_scales_short_quantities() -> None:
    """Test DOWN regime scales SHORT quantities by counter_qty_scale."""
    ...
    for i in range(len(short_shaped)):
        expected_qty = short_original[i].qty * 0.3
...
def test_truncation_keeps_closest_to_mid() -> None:
    ...
    for i in range(3):
        assert long_shaped[i].price == long_original[i].price
...
def test_counter_levels_zero_removes_side() -> None:
    ...
    # LONG should be empty (counter-trend in UP)
    assert len(long_ladder) == 0
    assert long_ladder.is_empty

    # SHORT should be full
    assert len(short_ladder) == 5
...
def test_counter_qty_scale_one_preserves_quantities() -> None:
    ...
    for i in range(len(short_shaped)):
        assert short_shaped[i].qty == pytest.approx(short_original[i].qty)
...
def test_prices_and_tpsl_preserved_after_throttling() -> None:
    ...
    for i in range(len(long_shaped)):
        assert long_shaped[i].price == long_original[i].price
        assert long_shaped[i].tp == long_original[i].tp
        assert long_shaped[i].sl == long_original[i].sl
        assert long_shaped[i].side == long_original[i].side
...
def test_core_and_scalp_strategy_behavior() -> None:
    ...
    assert len(long_up) == 2  # Throttled counter-trend
    assert len(short_up) == 5  # Full with-trend

    # Test DOWN regime
    shaped_down = PlacementPolicy.shape_ladders(original, Regime.DOWN, cfg)
    ...
    assert len(long_down) == 5  # Full with-trend
    assert len(short_down) == 2  # Throttled counter-trend
...
def test_throttled_counter_strategy_behavior() -> None:
    ...
    # LONG throttled, SHORT full (same as core-and-scalp implementation)
    assert len(long_ladder) == 3
    assert len(short_ladder) == 5
...
def test_extreme_qty_scaling() -> None:
    ...
    original = GridEngine.build_ladders(mid, cfg_small, Regime.SIDEWAYS)
    shaped = PlacementPolicy.shape_ladders(original, Regime.UP, cfg_small)

    long_shaped = next(ladder for ladder in shaped if ladder.side == Side.LONG)

    # Quantities should be 1% of original
    long_original = next(ladder for ladder in original if ladder.side == Side.LONG)
    for i in range(len(long_shaped)):
        expected_qty = long_original[i].qty * 0.01
        assert long_shaped[i].qty == pytest.approx(expected_qty)
